inlets = 3;
outlets = 1;
// inlet 1: num_columns = int 16 - 64, total number of columns
...
// actually runs process
	run(num_columns, max_row_fill_ratio, z_density);
}

function run(num_columns, max_row_fill_ratio, z_density) {
	var matrix_in_str = '';
	for ( var current_col_num = 0; current_col_num < num_columns; current_col_num++ ) {
		for ( var current_row_num = 0; current_row_num < 16; current_row_num++ ) {
			...
			matrix_in_str += current_col_num + ' ' + current_row_num + ' ' + current_cell_value + ' ';
		}
	}
